...
namespace DynamicBoltzmann {
	...
	********************/
	...
	// Interpolate a given 4 points (at x=-1,0,1,2) and a fraction between 0,1
	double BasisFunc2D::_cubic_interp(double *p, double f, int dim)
	{
		return p[1] + 0.5 * f*(p[2] - p[0] + f*(2.0*p[0] - 5.0*p[1] + 4.0*p[2] - p[3] + f*(3.0*(p[1] - p[2]) + p[3] - p[0])));
	};

	// Get the deriv of 4 points (at x=-1,0,1,2) and a fraction between 0,1
	double BasisFunc2D::_get_deriv(double *p, double f, int dim)
	{
		return 0.5 * (p[2] - p[0]) + 2.0 * f * (p[0] - 2.5 * p[1] + 2.0 * p[2] - 0.5 * p[3]) + 3.0 * f * f *( - 0.5 * p[0] + 1.5 * p[1] - 1.5 * p[2] + 0.5 * p[3]);
	};
	...
	double BasisFunc2D::_bicubic_interp(double x, double y, bool deriv_x, bool deriv_y)
	{
		...
		// i,j run from 1 to n-1
		// bounding box is 0 to n+1
		// So i-1,i,i+1,i+2 always exist
		...
		// The fraction to evaluate at
		double f;

		// Fraction for y
		// f = fmod(y,_delta2)/_delta2;
		// if (f > 1.0-1E-6) { f=0.0; }; // always use 0 rather than 1
		// if (_max2 - y < 1E-6) { f=1.0; }; // very end
		f = (y-(_min2+(j-1)*_delta2))/_delta2;
		if (f < 0.0 || f > 1.0) {
			std::cerr << "ERROR! fraction too great?" << std::endl; 
			exit(EXIT_FAILURE);
		};
		...
		p[3] = _valsb[i-1][j+2];
		if (deriv_y) {
			arr[0] = _get_deriv(p,f,1);
		} else {
			arr[0] = _cubic_interp(p,f,1);			
		};
		// i
		p[0] = _valsb[i][j-1];
		p[1] = _valsb[i][j];
		...
		p[3] = _valsb[i][j+2];
		if (deriv_y) {
			arr[1] = _get_deriv(p,f,1);
		} else {
			arr[1] = _cubic_interp(p,f,1);			
		};
		// i+1
		p[0] = _valsb[i+1][j-1];
		p[1] = _valsb[i+1][j];
		...
		p[3] = _valsb[i+1][j+2];
		if (deriv_y) {
			arr[2] = _get_deriv(p,f,1);
		} else {
			arr[2] = _cubic_interp(p,f,1);			
		};
		// i+2
		p[0] = _valsb[i+2][j-1];
		p[1] = _valsb[i+2][j];
		...
		p[3] = _valsb[i+2][j+2];
		if (deriv_y) {
			arr[3] = _get_deriv(p,f,1);
		} else {
			arr[3] = _cubic_interp(p,f,1);			
		};

		// Fraction for x
		f = (x-(_min1+(i-1)*_delta1))/_delta1;
		if (f < 0.0 || f > 1.0) {
			std::cerr << "ERROR! fraction too great?" << std::endl; 
			exit(EXIT_FAILURE);
		};
		...
		if (deriv_x) {
			return _get_deriv(arr,f,0);
		} else {
			return _cubic_interp(arr,f,0);
		};
	};
