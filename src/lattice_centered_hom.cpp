...
namespace dblz {
	...
	********************/
    ...
    LatticeCenteredHom::LatticeCenteredHom(int no_dims, int box_length, std::vector<Sptr> species_visible, LatticeCenteredHomMode mode) {
	...
	LatticeCenteredHom::LatticeCenteredHom(LatticeCenteredHom&& other) {
		_move(other);
	};
    ...
    int LatticeCenteredHom::_look_up_unit(int layer, int x) const {
        ...
        std::cerr << ">>> LatticeCenteredHom::_look_up_unit <<< could not find layer: " << layer << " x: " << x << std::endl;
        exit(EXIT_FAILURE);
    };
    int LatticeCenteredHom::_look_up_unit(int layer, int x, int y) const {
        ...
        std::cerr << ">>> LatticeCenteredHom::_look_up_unit <<< could not find layer: " << layer << " x: " << x << " y: " << y << std::endl;
        exit(EXIT_FAILURE);
    };
    int LatticeCenteredHom::_look_up_unit(int layer, int x, int y, int z) const {
        ...
        std::cerr << ">>> LatticeCenteredHom::_look_up_unit <<< could not find layer: " << layer << " x: " << x << " y: " << y << " z: " << z << std::endl;
        exit(EXIT_FAILURE);
    };
    
    std::vector<int> LatticeCenteredHom::_look_up_pos(int layer, int idx) const {
    ...
    int LatticeCenteredHom::get_no_dims() const {
        return _no_dims;
    };
    
    int LatticeCenteredHom::get_box_length() const {
        return _box_length;
    };
    
    int LatticeCenteredHom::get_no_units_in_layer(int layer) const {
        return _no_units_per_layer.at(layer);
    };
    
    int LatticeCenteredHom::get_no_layers() const {
        return _no_layers;
    };
    
    LatticeCenteredHomMode LatticeCenteredHom::get_LatticeCenteredHom_mode() const {
        return _mode;
    };
    ...
    int LatticeCenteredHom::get_no_markov_chains(MCType type) const {
        return _no_markov_chains.at(type);
    };

    void LatticeCenteredHom::set_no_markov_chains(MCType type, int no_markov_chains) {
    ...
    void LatticeCenteredHom::add_layer(int layer, int box_length, std::vector<Sptr> species) {
        ...
        // Centered?
        if (_mode == LatticeCenteredHomMode::NORMAL_W_CENTERED_GRADIENT_VEC) {
        ...
        } else if (_mode == LatticeCenteredHomMode::NORMAL_W_CENTERED_GRADIENT_PT || _mode == LatticeCenteredHomMode::CENTERED_PT) {
            for (auto sp: _species_possible_vec.at(layer)) {
	...
	********************/
    ...
    // Biases
    void LatticeCenteredHom::set_bias_of_layer(int layer, Sptr sp, Iptr bias) {
    ...
    // Ixns
    void LatticeCenteredHom::set_ixn_between_layers(int layer1, Sptr sp1, int layer2, Sptr sp2, Iptr ixn) {
        if (layer2 != layer1+1 && layer2 != layer1-1) {
            std::cerr << ">>> LatticeCenteredHom::add_ixn_between_layers <<< layer2 != layer1 +- 1; instead layer_2 = " << layer2 << " and layer1 = " << layer1 << std::endl;
            exit(EXIT_FAILURE);
        };
    ...
    // Set multiplier
    void LatticeCenteredHom::set_multiplier_between_layers(int from_layer, int to_layer, double multiplier) {
        if (to_layer != from_layer+1 && to_layer != from_layer-1) {
            std::cerr << ">>> LatticeCenteredHom::set_multiplier <<< to_layer != from_layer +- 1; instead layer_2 = " << to_layer << " and from_layer = " << from_layer << std::endl;
            exit(EXIT_FAILURE);
        };
        
        _o2_mults[from_layer][to_layer] = multiplier;
    };
    void LatticeCenteredHom::set_multiplier_for_bias_in_layer(int layer, double multiplier) {
        _bias_mults[layer] = multiplier;
    };
    
    // Get ixns
    double LatticeCenteredHom::get_bias_in_layer(int layer, Sptr sp) const {
    ...
    double LatticeCenteredHom::get_ixn_between_layers(int from_layer, Sptr from_sp, int to_layer, Sptr to_sp) const {
        if (to_layer != from_layer+1 && to_layer != from_layer-1) {
            std::cerr << ">>> LatticeCenteredHom::add_ixn_between_layers <<< to_layer != from_layer +- 1; instead to_layer = " << to_layer << " and from_layer = " << from_layer << std::endl;
            exit(EXIT_FAILURE);
        };
        ...
        if (itm != _o2_mults.end()) {
            ...
            if (itm2 != itm->second.end()) {
                mult = itm2->second;
                // std::cout << "LatticeCenteredHom::get_ixn_between_layers: from_layer = " << from_layer << " to to_layer = " << to_layer << " mult = " << mult << std::endl;
            };
        };
    ...
    // Get all ixns
    const std::vector<Iptr>& LatticeCenteredHom::get_all_ixn_params() const {
        return _all_ixns;
    };
	...
	********************/
    ...
    void LatticeCenteredHom::add_conn(int layer1, int x1, int y1, int layer2, int x2, int y2) {
        int idx1 = _look_up_unit(layer1, x1, y1);
        int idx2 = _look_up_unit(layer2, x2, y2);
        _adj[layer1][layer2](idx2,idx1) = 1.0;
        _adj[layer2][layer1](idx1,idx2) = 1.0;
    };
    void LatticeCenteredHom::add_conn(int layer1, int x1, int y1, int z1, int layer2, int x2, int y2, int z2) {
	...
	********************/
	...
	void LatticeCenteredHom::set_empty_all_units(MCType chain, int i_chain) {
        for (auto layer=0; layer<_no_layers; layer++) {
            set_empty_all_units_in_layer(chain, i_chain, layer);
        };
	};
    void LatticeCenteredHom::set_empty_all_units_in_layer(MCType chain, int i_chain, int layer) {
        for (auto sp: _species_possible_vec[layer]) {
            _mc_chains[chain][i_chain][layer][sp].fill(0.0);
        };
    };
    void LatticeCenteredHom::set_empty_all_hidden_units(MCType chain, int i_chain) {
    ...
    // Random
    void LatticeCenteredHom::set_random_all_units(MCType chain, int i_chain, bool binary) {
        for (auto layer=0; layer<_no_layers; layer++) {
            set_random_all_units_in_layer(chain, i_chain, layer,binary);
        };
    };
    void LatticeCenteredHom::set_random_all_units_in_layer(MCType chain, int i_chain, int layer, bool binary) {
        // Random probs
        for (auto sp: _species_possible_vec.at(layer)) {
    ...
    void LatticeCenteredHom::set_random_all_hidden_units(MCType chain, int i_chain, bool binary) {
        for (auto layer=1; layer<_no_layers; layer++) {
            set_random_all_units_in_layer(chain,i_chain,layer,binary);
        };
    };
    ...
    // Binarize
    void LatticeCenteredHom::binarize_all_units(MCType chain, int i_chain) {
        for (auto layer=0; layer<_no_layers; layer++) {
            binarize_all_units_in_layer(chain,i_chain,layer);
        };
    };
    void LatticeCenteredHom::_binarize_all_units_in_layer(MCType chain, int i_chain, int layer, bool act) {
        // Random vec
    ...
    void LatticeCenteredHom::binarize_all_units_in_layer(MCType chain, int i_chain, int layer) {
        _binarize_all_units_in_layer(chain,i_chain,layer,false);
    };
    void LatticeCenteredHom::binarize_all_hidden_units(MCType chain, int i_chain) {
        for (auto layer=1; layer<_no_layers; layer++) {
            binarize_all_units_in_layer(chain,i_chain,layer);
        };
    };
	...
	********************/
	...
	void LatticeCenteredHom::write_layer_to_file(MCType chain, int i_chain, int layer, std::string fname, bool binary) const
	{
		std::ofstream f;
		f.open (fname);
		if (!f.is_open()) { // make sure we found it
			std::cerr << ">>> LatticeCenteredHom::write_layer_to_file <<< Error: could not open file: " << fname << " for writing" << std::endl;
			exit(EXIT_FAILURE);
		};
	...
	void LatticeCenteredHom::read_layer_from_file(MCType chain, int i_chain, int layer, std::string fname, bool binary)
	{
		// Clear the layer
        set_empty_all_units_in_layer(chain, i_chain, layer);
		...
		f.open(fname);
		if (!f.is_open()) { // make sure we found it
			std::cerr << ">>> Error: LatticeCenteredHom::read_layer_from_file <<< could not find file: " << fname << std::endl;
			exit(EXIT_FAILURE);
		};
        ...
        if (binary) {
            ...
            for (auto const &pr: occs_to_write_binary) {
                ...
                if (it == _species_possible_map[layer].end()) {
                    std::cerr << ">>> LatticeCenteredHom::read_from_file <<< Error: could not find species (binary): " << pr.first << std::endl;
                    exit(EXIT_FAILURE);
                };
            ...
            for (auto const &pr: occs_to_write_prob) {
                ...
                auto it = _species_possible_map[layer].find(pr.first);
                if (it == _species_possible_map[layer].end()) {
                    std::cerr << ">>> LatticeCenteredHom::read_from_file <<< Error: could not find species (prob): " << pr.first << std::endl;
                    exit(EXIT_FAILURE);
                };
    ...
    void LatticeCenteredHom::activate_layer_calculate_from_below(MCType chain, int layer) {
        
        if (_mode == LatticeCenteredHomMode::NORMAL || _mode == LatticeCenteredHomMode::NORMAL_W_CENTERED_GRADIENT_PT || _mode == LatticeCenteredHomMode::NORMAL_W_CENTERED_GRADIENT_VEC) {
        ...
        } else if (_mode == LatticeCenteredHomMode::CENTERED_PT) {
    ...
    void LatticeCenteredHom::activate_layer_calculate_from_above(MCType chain, int layer) {
    ...
    void LatticeCenteredHom::activate_layer_calculate_from_both(MCType chain, int layer) {
    ...
    void LatticeCenteredHom::activate_layer_convert_to_probs(MCType chain, int layer, bool binary) {
    ...
    void LatticeCenteredHom::activate_layer_committ(MCType chain, int layer) {
        // All chains
        for (auto i_chain=0; i_chain<_no_markov_chains.at(chain); i_chain++) {
            for (auto sp: _species_possible_vec.at(layer)) {
    ...
    void LatticeCenteredHom::mean_field_hiddens_step() {
    ...
    // Sample
    void LatticeCenteredHom::gibbs_sampling_step_awake(bool binary_hidden) {
    ...
    void LatticeCenteredHom::gibbs_sampling_step_parallel_awake(bool binary_hidden) {
    ...
    // Sample
    void LatticeCenteredHom::gibbs_sampling_step(bool binary_visible, bool binary_hidden) {
    ...
    void LatticeCenteredHom::gibbs_sampling_step_parallel(bool binary_visible, bool binary_hidden) {
    ...
    void LatticeCenteredHom::activate_upward_pass(MCType chain, bool binary_hidden) {
    ...
    void LatticeCenteredHom::activate_upward_pass_with_2x_weights_1x_bias(MCType chain, bool binary_hidden) {
    ...
    void LatticeCenteredHom::reap_moments_and_slide_centers_normal() {
    ...
    void LatticeCenteredHom::reap_moments_and_slide_centers_normal_w_centered_gradient_vec(bool slide_means, double sliding_factor) {
    ...
    void LatticeCenteredHom::reap_moments_and_slide_centers_normal_w_centered_gradient_pt(bool slide_means, double sliding_factor) {
    ...
    void LatticeCenteredHom::reap_moments_and_slide_centers_centered_pt(bool slide_means, double sliding_factor) {
    ...
    void LatticeCenteredHom::read_centers_from_file(int layer, std::string fname) {
        // Open
        std::ifstream f;
        f.open(fname);
        if (!f.is_open()) { // make sure we found it
            std::cerr << ">>> Error: LatticeCenteredHom::read_centers_from_file <<< could not find file: " << fname << std::endl;
            exit(EXIT_FAILURE);
        };
    ...
    void LatticeCenteredHom::write_centers_to_file(int layer, std::string fname) const {
        std::ofstream f;
        f.open (fname);
        if (!f.is_open()) { // make sure we found it
            std::cerr << ">>> LatticeCenteredHom::write_centers_to_file <<< Error: could not open file: " << fname << " for writing" << std::endl;
            exit(EXIT_FAILURE);
        };
    ...
    void LatticeCenteredHom::read_center_pts_from_file(std::string fname) {
        ...
        while (getline(f,line)) {
            ...
            if (sp != "") {
                ...
                // std::cout << "LatticeCenteredHom::read_center_pt_from_file: Set layer: " << layer << " species: " << species->get_name() << " to center: " << _cpt_means.at(layer).at(species) << std::endl;
            };
            sp=""; center=""; layer_str="";
        };
    ...
    arma::vec LatticeCenteredHom::get_center_vec_for_species_in_layer(int layer, Sptr species) const {
        return _c_means.at(layer).at(species);
    };
    void LatticeCenteredHom::set_center_vec_for_species_in_layer(int layer, Sptr species, arma::vec center) {
        _c_means[layer][species] = center;
    };
    void LatticeCenteredHom::set_center_vec_for_species_in_layer(int layer, Sptr species, double center) {
        int no_units = get_no_units_in_layer(layer);
    ...
    void LatticeCenteredHom::_add_to_all_ixns_vec(Iptr ixn) {
    ...
    void LatticeCenteredHom::wake_sleep_loop_bm_pcd(int i_opt_step, int no_mean_field_updates, int no_gibbs_sampling_steps, std::vector<FName> &fnames, OptionsWakeSleep_BM_PCD options) {
        ...
        // Write out the LatticeCenteredHoms
    ...
    double LatticeCenteredHom::get_count_1d(MCType chain, int i_chain, Sptr sp) const {
        if (_no_dims != 1) {
            std::cerr << ">>> LatticeCenteredHom::get_count_1d <<< Error: only for 1D LatticeCenteredHoms" << std::endl;
            exit(EXIT_FAILURE);
        };
        
        return arma::accu(_mc_chains.at(chain).at(i_chain).at(0).at(sp));
    };
    double LatticeCenteredHom::get_count_1d(MCType chain, int i_chain, Sptr sp1, Sptr sp2) const {
        if (_no_dims != 1) {
            std::cerr << ">>> LatticeCenteredHom::get_count_1d <<< Error: only for 1D LatticeCenteredHoms" << std::endl;
            exit(EXIT_FAILURE);
        };
    ...
    double LatticeCenteredHom::get_count_1d(MCType chain, int i_chain, Sptr sp1, Sptr sp2, Sptr sp3) const {
        if (_no_dims != 1) {
            std::cerr << ">>> LatticeCenteredHom::get_count_1d <<< Error: only for 1D LatticeCenteredHoms" << std::endl;
            exit(EXIT_FAILURE);
        };
    ...
    double LatticeCenteredHom::get_count_1d(MCType chain, int i_chain, Sptr sp1, Sptr sp2, Sptr sp3, Sptr sp4) const {
        if (_no_dims != 1) {
            std::cerr << ">>> LatticeCenteredHom::get_count_1d <<< Error: only for 1D LatticeCenteredHoms" << std::endl;
            exit(EXIT_FAILURE);
        };
