...
namespace dblz {
	...
	********************/
	...
	void Unit::form_propensity_vector_at_timepoint(int timepoint) {
	...
	void Unit::form_propensity_vector_at_timepoint(int timepoint, int given_layer) {
		for (auto i=0; i<_activations.size(); i++) {
			_activations[i].act = 0.0;
			_activations[i].prob = exp(_activations[i].act);
			_activations[i].prop = _activations[i-1].prop + _activations[i].prob;
	...
	void Unit::prepare_sample_at_timepoint(int timepoint, bool binary) {
		// Form the activations vector
		form_propensity_vector_at_timepoint(timepoint);
		// Sample
		_prepare_sample(binary);
	};

	void Unit::prepare_sample_at_timepoint(int timepoint, bool binary, int given_layer) {
		// Form the activations vector
	...
	********************/
	...
	void UnitVisible::form_propensity_vector_at_timepoint(int timepoint) {
		for (auto i=1; i<_activations.size(); i++) {
			...
			// Bias
			if (_bias_dict) {
				_activations[i].act += _bias_dict->get_ixn_at_timepoint(_activations[i].sp,timepoint);
			};

			// VV conns
			for (auto const &conn_vv: _conns_vv) {
				_activations[i].act += conn_vv.first->get_act_for_species_at_unit_at_timepoint(_activations[i].sp,conn_vv.second,timepoint);
			};

			// VVV conns
			for (auto const &conn_vvv: _conns_vvv) {
				_activations[i].act += conn_vvv.first->get_act_for_species_at_unit_at_timepoint(_activations[i].sp,conn_vvv.second,timepoint);
			};

			// VH conns
			for (auto const &conn_vh: _conns_vh) {
				_activations[i].act += conn_vh->get_act_for_species_at_unit_v_at_timepoint(_activations[i].sp,timepoint);
			};
	...
	void UnitVisible::form_propensity_vector_at_timepoint(int timepoint, int given_layer) {
		if (given_layer == 1) {
	...
	********************/
	...
	void UnitHidden::form_propensity_vector_at_timepoint(int timepoint) {
		...
		for (auto i=1; i<_activations.size(); i++) {
			...
			// VH conns
			for (auto const &conn_vh: _conns_vh) {
				_activations[i].act += conn_vh->get_act_for_species_at_unit_h_at_timepoint(_activations[i].sp,timepoint);
			};
			...
			for (auto layer: _conns_hh) {
				...
				for (auto const &conn_hh: layer.second) {
					_activations[i].act += conn_hh.first->get_act_for_species_at_unit_at_timepoint(_activations[i].sp,conn_hh.second,timepoint);
				};
			};
	...
	void UnitHidden::form_propensity_vector_at_timepoint(int timepoint, int given_layer) {
